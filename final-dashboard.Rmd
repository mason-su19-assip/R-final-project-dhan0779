...
leaflet(slice(dfnasa,1:576)) %>%
  ...
  addCircleMarkers(lat = dflat, lng = dflong,color = ~qpal(temperature),stroke = FALSE, fillOpacity= 0.8)
```
...
qpal <- colorFactor(c("green","yellow"), domain = dfnasa$pressure[1:576])

leaflet(slice(dfnasa,1:576)) %>%
  addTiles() %>%
  setView(lng = -90, lat =  10, zoom = 3) %>%
  addCircleMarkers(lat = dflat, lng = dflong,color = ~qpal(pressure),stroke = FALSE, fillOpacity= 0.6)
```
